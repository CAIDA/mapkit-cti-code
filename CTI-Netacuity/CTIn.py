...
aspath_filename = "/project/mapkit/agamerog/country_asn_analysis/singlecountryfiles/" + current_country + "." + current_snapshot + ".txt.bz2" + testing_string
...
country_filename = "/project/mapkit/agamerog/country_asn_analysis/country_aspath/" + current_snapshot + "/" + current_country + ".cti.stateowned.csv"
...
def path_cut_peak(path, asrel_set, stateowned_ases):
    ...
    if not ignore_path: #now that we know where the path ends, replace highest ranking transit with '88888'
        #if it is a transit provider. if not try next.
        split_path = path.split('|')
        out_path = ''
        for i in reversed(range( (len(split_path) ) )):
            if i == (len(split_path) - 1):
                out_path = split_path[i]
                continue #no need to replace the origin
            possible_state = split_path[i]
            if possible_state in stateowned_ases:
                #only replace transit in path if the provider is in the CTI set
                #print('FOUND STATE OWNED')
                print(stateowned_ases)
                print(possible_state)
                out_path = '88888' + '|' + out_path
                break #we found our state-owned boy, go home
            else:
                 out_path = split_path[i] + '|' + out_path
...
def read_approved_targets(): #read targeted ASes, except those in the multi-as set
    exclude_set = set()
    with open('/project/mapkit/agamerog/country_asn_analysis/stateowned/stateowned_cti_domestic_only.csv','r+') as g:
        try:
            ipmap_list = g.readlines()
        except:
            sys.stderr.write("\n something went wrong opening /project/mapkit/agamerog/country_asn_analysis/stateowned_cti_domestic_only.csv \n")
            sys.exit()
        for i in range(len(ipmap_list)):
            row = ipmap_list[i].strip('\n')
            if '#' in row:
            ...
            if rowCountry != current_country:
                continue
            rowAs = row.split(',')[2].split('-')[0]
            exclude_set.add(rowAs)
...
def read_as_path_file(filename, country_set, asrel_set, mon_dict, global_origin_as_dict, prefix_origin):
    ...
    print(filename)
    global_approved_asns, nothing = read_approved_targets()
    stateowned_ases = set(['3549, 3356'])
    print_dict = dict()
    as_dict = dict()
    pfix_dict = dict()
    ...
    for i in range(len(ipmap_list)):
        ...
        origpath = path
        realorigin = path.split('|')[-1]
        collector = path.split('|')[0]
        #collector = path
 
        path_components = path.split('|')
        checking_origin_asn = path_components[-1]
        #if checking_origin_asn not in global_approved_asns: #THese two lines
        #Removed on Oct. 5 to adapt to new BGP only CTI def
        ...
        new_path = path_cut_peak(path, asrel_set, stateowned_ases)
        if len(new_path) == 0:
        ...
        outpath = collector + '|' + new_path #path including origin and transit providers
        #print('OUTPUT ' + str(outpath))
        #print('*********')
  
        try: #append to tree key if it exists
            node.data[mon].append([outpath, collector, realorigin]) #save path and BGP host AS ("collector")
        except KeyError:
            node.data[mon] = [[outpath, collector, realorigin]] #create list with a sinble path and collector
        #DEBUG
                ...
                if this_level < curr_level:
                    ...
                    if mon in used_lengths: #see if this monitor has seen a prefix in this tree (for overlap discount)
                        ...
                        as_dict, pfix_dict, origin_ases, transit_ases, parsed_prefix_collectors, this_weight =  \
                                ...
                                as_dict, pfix_dict, prefix_collector, tree_prefix, parsed_prefix_collectors, stateowned_ases)
...
def update_matrices(monitor_node, current_weight, used_length, prefix_length, origin_ases, transit_ases, as_dict, pfix_dict, prefix_collector, tree_prefix, parsed_prefix_collectors, stateowned_ases):
    ...
    for i in range(len(monitor_node)):
        ...
        for i in range(len(components)):
            if i == 0:
                continue #ignore host AS #This AS is now slashed later in the path
        ...
        for i in range(len(transit_list)):
            ...
            if key in pfix_dict: 
                ...
                if originreal in stateowned_ases and transit_list[i][0] == '88888': #don't bill this CTI as it is originated by the state
                    pfix_dict[key][0] = float(pfix_dict[key][0])
                else:
                ...
                if transit_list[i][1] != 1:
                    if originreal in stateowned_ases and transit_list[i][0] == '88888': #don't bill this CTI as it is originated by the state
                        ...
                        pfix_dict[key] = [0.0, [transit_list[i][1]], 1, dict()]
                    else:
                        pfix_dict[key] = [this_prefix_length*transit_weight, [transit_list[i][1]], 1, dict()]
                    
                    pfix_dict[key][3][pos_for_count] = 1
                else:
                    if originreal in stateowned_ases and transit_list[i][0] == '88888':
                        pfix_dict[key] = [0.0, [0], 0, dict()]
                    else:
