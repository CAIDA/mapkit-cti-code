...
countries = ['BD', 'BE', 'BF', 'BG', 'BA', 'BO', 'JP', 'BI', 'BJ', 'BT', 'JM', 'BW', 'BR', 'CR', 'BY', 'RU', 'RW', 'RS', 'LT', 'LU', 'LR', 'RO', 'GW', 'GT', 'GR', 'GQ', 'BH', 'GY', 'GB', 'GA', 'GN', 'GM', 'GH', 'OM', 'JO', 'HR', 'HT', 'HU', 'HN', 'PS', 'PT', 'PY', 'PA', 'PG', 'PE', 'PK', 'PH', 'PL', 'ZM', 'EE', 'EG', 'ZA', 'EC', 'IT', 'AO', 'ET', 'ZW', 'ES', 'ER', 'ME', 'MD', 'MG', 'MA', 'UZ', 'MM', 'ML', 'MN', 'US', 'MU', 'MT', 'MW', 'MR', 'UG', 'UA', 'MX', 'IL', 'FR', 'FI', 'FJ', 'NI', 'NL', 'NO', 'NE', 'NG', 'NZ', 'NP', 'CH', 'CO', 'CN', 'CM', 'CL', 'CA', 'CG', 'CF', 'CD', 'CZ', 'CY', 'UY', 'CV', 'CU', 'SZ', 'SY', 'KG', 'SR', 'KH', 'KM', 'SK', 'KR', 'SI', 'KP', 'KW', 'SL', 'KZ', 'SA', 'SG', 'SE', 'SD', 'DO', 'DJ', 'DK', 'DE', 'YE', 'DZ', 'MK', 'LV', 'TZ', 'LA', 'TT', 'TR', 'LK', 'TN', 'TL', 'TM', 'TJ', 'LS', 'TH', 'TG', 'TD', 'LY', 'AE', 'VE', 'AF', 'IQ', 'IS', 'IR', 'AM', 'AL', 'VN', 'AR', 'AU', 'AT', 'IN', 'LB', 'AZ', 'IE', 'ID', 'MY', 'QA', 'MZ']

plot_filename = "/project/mapkit/agamerog/country_asn_analysis/plots/democracy_scatter_" + snapshot + ".pdf"
...
def read_top_file(country):
    origin_filename = "/project/mapkit/agamerog/country_asn_analysis/country_aspath/" + snapshot + "/top." + country + ".csv.bz2"
    with bz2.BZ2File(origin_filename, 'rb') as f: #import file
        ipmap_list = f.readlines()
...
def read_gdp_file():
    ...
    summary_filename = "DemocracyIndex2017.csv"
    with open(summary_filename, 'rb') as f: #import file
        ipmap_list = f.readlines()
        print "IM HERE"
        for i in range(len(ipmap_list)):
...
def plot_origin_transit(country_numbers, country_dict, country_ases, cc_dict, gdp_dict):
    ...
    for country in country_numbers:
        ...
        ax.set_xlabel('Democracy Index - Political Participation Score', fontsize = 32)
        csize = z 
        y1.append(x)
        y2.append(y)
        xx.append(csize)
    ...
    xxs = sorted(xx)
    #ax.scatter(xxs,y1s,color='r', alpha = 0.7, label = 'Percentage of ASes')
    ax.scatter(xxs,y2s,color='b',alpha = 0.7, label = 'Percentage of Addresses')
    ...
    print('Number of countries = ' +str(len(xxs)))
